...
class OrderRisksEndpoint extends AbstractEndpoint
{
    ...
    {
        $request = new GetJson('orders/' . $orderId . '/risks.json');
    ...
    {
        $request = new GetJson('orders/' . $orderId . '/risks/' . $orderRisksId . '.json');
        $response = $this->send($request);
    ...
    {
        $request = new PostJson('orders/' . $orderId . '/risks.json', $orderRisks);
        $response = $this->send($request);
        return $this->createEntity($response->get('risk'));
    }
    ...
    {
        $request = new PutJson('orders/' . $orderId . '/risks/' . $orderRisksId . '.json', array('risk' => $orderRisks->toArray()));
        $response = $this->send($request);
        return $this->createEntity($response->get('risk'));
    }
    ...
    {
        $request = new DeleteParams('orders/' . $orderId . '/risks/' . $orderRisksId . '.json');
        $this->send($request);
    }
}
