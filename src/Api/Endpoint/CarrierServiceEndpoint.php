...
class CarrierServiceEndpoint extends AbstractEndpoint
{
    ...
    {
        $request = new GetJson('carrier_services.json');
    ...
    {
        $request = new GetJson('carrier_services/' . $carrierServiceId . '.json');
        $response = $this->send($request);
    ...
    {
        $request = new PostJson('carrier_services.json', array('carrier_service' => $carrierService->toArray()));
        $response = $this->send($request);
        return $this->createEntity($response->get('carrier_service'));
    }
    ...
    {
        $request = new PostJson('carrier_services/' . $carrierServiceId . '.json', array('carrier_service' => $carrierService->toArray()));
        $response = $this->send($request);
        return $this->createEntity($response->get('carrier_service'));
    }
    ...
    {
        $request = new DeleteParams('carrier_services/' . $carrierServiceId . '.json');
        $this->send($request);
    }
}
