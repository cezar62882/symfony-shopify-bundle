...
class TransactionEndpoint extends AbstractEndpoint
{
    ...
    {
        $request = new GetJson('orders/' . $orderId . '/transactions.json', $query);
    ...
    {
        $request = new GetJson('orders/' . $orderId . '/transactions/count.json');
        $response = $this->send($request);
    ...
    public function findOne($orderId, $transactionId, array $fields = array())
    {
        ...
        $request = new GetJson('orders/' . $orderId . '/transactions/' . $transactionId . '.json', $params);
        $response = $this->send($request);
    ...
    public function capture($orderId, $amount = null)
    {
        ...
        $request = new PostJson('orders/' . $orderId . '/transactions.json', array('transaction' => $params));
        $this->send($request);
    }
}
