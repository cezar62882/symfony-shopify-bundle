...
class ApplicationChargeEndpoint extends AbstractEndpoint
{
    ...
    {
        $request = new GetParams('application_charges.json', $query);
    ...
    {
        $request = new GetParams('application_charges/' . $applicationChargeId . '.json');
        $response = $this->send($request);
    ...
    {
        $request = new PostJson('application_charges.json', array('application_charge' => $applicationCharge->toArray()));
        $response = $this->send($request);
        return $this->createEntity($response->get('application_charge'));
    }
    ...
    {
        $request = new PostJson('application_charges/' . $applicationChargeId . '/activate.json', null);
        $this->send($request);
    }
}
