...
class RecurringApplicationChargeEndpoint extends AbstractEndpoint
{
    ...
    {
        $request = new PostJson('recurring_application_charges.json', array('recurring_application_charge' => $recurringApplicationCharge->toArray()));
    ...
    {
        $request = new GetJson('recurring_application_charges/' . $recurringApplicationChargeId . '.json');
        $response = $this->send($request);
        return $this->createEntity($response->get('recurring_application_charge'));
    }
    ...
    {
        $request = new GetJson('recurring_application_charges.json', $query);
        $response = $this->send($request);
    ...
    {
        $request = new PostJson('recurring_application_charges/' . $recurringApplicationChargeId . '/activate.json');
        $response = $this->send($request);
        return $this->createEntity($response->get('recurring_application_charge'));
    }
    ...
    {
        $request = new DeleteParams('recurring_application_charges/' . $recurringApplicationChargeId . '.json');
        $this->send($request);
    }
}
