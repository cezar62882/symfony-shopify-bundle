...
class ThemeEndpoint extends AbstractEndpoint
{
    ...
    public function findAll(array $fields = array())
    {
        ...
        $request = new GetJson('themes.json', $params);
    ...
    {
        $params = $fields ? array('fields' => implode(',', $fields)) : array();
        $request = new GetJson('themes/' . $themeId . '.json', $params);
        $response = $this->send($request);
    ...
    {
        $request = new PostJson('themes.json', array('theme' => $theme->toArray()));
        $response = $this->send($request);
    ...
    {
        $request = new PutJson('themes/' . $themeId . '.json', array('theme' => $theme->toArray()));
        $response = $this->send($request);
        return $this->create($response->get('theme'));
    }
    ...
    {
        $request = new DeleteParams('themes/' . $themeId . '.json');
        $this->send($request);
    }
}
