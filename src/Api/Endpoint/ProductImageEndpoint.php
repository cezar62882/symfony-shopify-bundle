...
class ProductImageEndpoint extends AbstractEndpoint
{
    ...
    {
        $request = new GetJson('products/' . $productId . '/images.json', $query);
    ...
    {
        $request = new GetJson('products/' . $productId . '/images/count.json', $query);
        $response = $this->send($request);
    ...
    public function findOne($productId, $productImageId, array $fields = array())
    {
        ...
        $request = new GetJson('products/' . $productId . '/images/' . $productImageId . '.json', $params);
        $response = $this->send($request);
    ...
    {
        $request = new PostJson('products/' . $productId . '/images.json', array('image' => $productImage->toArray()));
        $response = $this->send($request);
        return $this->createEntity($response->get('image'));
    }
    ...
    {
        $request = new PutJson('products/' . $productId . '/images/' . $productImageId . '.json', array('image' => $productImage->toArray()));
        $response = $this->send($request);
        return $this->createEntity($response->get('image'));
    }
    ...
    {
        $request = new DeleteParams('products/' . $productId . '/images/' . $productImageId . '.json');
        $this->send($request);
    }
}
