...
class FulfillmentServiceEndpoint extends AbstractEndpoint
{
    ...
    {
        $request = new GetJson('fulfillment_services.json', $query);
    ...
    {
        $request = new GetJson('fulfillment_services/' . $fulfillmentServiceId . '.json');
        $response = $this->send($request);
    ...
    {
        $request = new PostJson('fulfillment_services.json', array('fulfillment_service' => $fulfillmentService->toArray()));
        $response = $this->send($request);
        return $this->createEntity($response->get('fulfillment_service'));
    }
    ...
    {
        $request = new PutJson('fulfillment_services/' . $fulfillmentServiceId . '.json', array('fulfillment_service' => $fulfillmentService->toArray()));
        $response = $this->send($request);
        return $this->createEntity($response->get('fulfillment_service'));
    }
    ...
    {
        $request = new DeleteParams('fulfillment_services/' . $fulfillmentServiceId . '.json');
        $this->send($request);
    }
}
